...
const UserManagementPage = () => {
  ...
  const handleEditUser = async (e: any) => {
    e.preventDefault();
    try {
      await axios.put(`/user/update/${currentUser.username}`, formData, {
        headers: {
          Authorization: `Bearer ${cookies.auth?.token}`
        }
      });
  ...
  const handleResetPassword = async (e: any) => {
    e.preventDefault();
    try {
      await axios.put(`/user/reset-password/${currentUser.username}`, { newPassword: formData.password }, {
        headers: {
          Authorization: `Bearer ${cookies.auth?.token}`
        }
      });
  ...
  const toggleStatus = async (user: any) => {
    try {
      await axios.put(`/user/toggle-status/${user.username}`, { isActive: !user.isActive }, {
        headers: {
          Authorization: `Bearer ${cookies.auth?.token}`
        }
      });
